...
// Finde Substring in String (ist der substring da oder nicht?) returnt 1 wenn
// er da ist, 0 wenn nicht
int findstr(char *string, char *sub_string) {
  int l1 = len(string);
  int l2 = len(sub_string);
  int window_start = 0;
...
float convertfloatstr(char *float_string) {
  ...
  int n = breakpoint - 1;
  for (int j = 0; j < breakpoint; j++) {
    curr_num = curr_num + (float_string[j] - '0') * mypow(10, n);
    n--;
  }
  float curr_float = 0;
  int n2 = 1;
  for (int k = breakpoint + 1; k < l; k++) {
    curr_float = curr_float + (float_string[k] - '0') * negativemypow(10, n2);
    n2++;
  }
